...
public final class LoanContractAndSalaryVerifier {

    private static final int FMF_ZSTD_OFFSET = 26;

    private LoanContractAndSalaryVerifier() {
    }

    public static void main(String[] args) throws Exception {
        Path csv = args.length > 0 ? Path.of(args[0]) : Path.of("loan_contract_and_salary.csv");
        byte[] payload = loadPayload(Path.of("games/Feyenoord_after.fm"));
        List<Row> rows = readCsv(csv);

        int fullyMatched = 0;
        List<String> contractWrong = new ArrayList<>();
        ...
        for (Row row : rows) {
            ...
            if (best == null || best.contractEnd() == null || best.contractEnd().date() == null || best.salary() == null) {
                missing.add(row.id() + " " + row.name());
                continue;
            }
            boolean contractOk = best.contractEnd().date().equals(row.contractEndDate());
            int roundedSalary = roundForDisplay(best.salary().value());
            boolean salaryOk = roundedSalary == row.salaryPerWeek();

            if (contractOk && salaryOk) {
                fullyMatched++;
                continue;
            }
            if (!contractOk) {
                contractWrong.add(row.id() + " " + row.name()
                        + " expected=" + row.contractEndDate()
                        + " actual=" + best.contractEnd().date());
            }
            if (!salaryOk) {
                salaryWrong.add(row.id() + " " + row.name()
                        + " expected=" + row.salaryPerWeek()
                        + " actual=" + roundedSalary
                        + " raw=" + best.salary().value());
            }
        }

        System.out.println("checked=" + rows.size());
        System.out.println("fullMatches=" + fullyMatched);
        System.out.println("contractWrong=" + contractWrong.size());
        for (String line : contractWrong) {
            System.out.println("CONTRACT_WRONG " + line);
        }
    ...
    private record Row(int id, String name, LocalDate contractEndDate, int salaryPerWeek) {
    }
}
